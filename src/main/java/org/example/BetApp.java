...
public class BetApp {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int j = 0; j < urls.size(); j++) {
            ...
            List<WebElement> containers = getContainers(driver);
            ...
            for (int l = 1; l <= containers.size(); l++) {
                ...
                List<WebElement> expandButton = driver.findElements(By.xpath(String.format("(//div[@class='bet-card__footer']/button)[%d]", l)));
                ...
                List<WebElement> divisions = getDivisions(driver, l);
                for (int k = 1; k <= divisions.size(); k++) {
                    String league = "(" + CONTAINER_EL + LEAGUE_EL + ")[" + k + "]";

                    String divisionName = getIfPresentNotRoot(driver, league + "//div[@class='subpage-bet-row__title-row__sport-title']/h3");
                    log("---- Fetching data for division " + k + "(" + divisionName + ")");
                    List<WebElement> games = driver.findElements(By.xpath(league + GAME_EL));
                    log("------ Found games - " + games.size());
                    for (int i = 1; i <= games.size(); i++) {
                        log("-------- Fetching data for game " + i);
                        BetResult betResult = new BetResult(urls.get(j));
                        String teamA = getIfPresent(driver, i, "//span[contains(@class, 'team--home')]");
                        betResult.setTeamA(teamA);
                        betResult.setTeamB(getIfPresent(driver, i, "//span[contains(@class, 'team--away')]"));

                        betResult.setDate(getIfPresent(driver, i, "//span[contains(@class, 'date-label')]"));
                        betResult.setTime(getIfPresent(driver, i, "//div[contains(@class, 'time-label')]/span[2]"));
                        betResult.setOdsFor1(getIfPresent(driver, i, "//div[contains(@class, 'buttons-three-columns')]/button[1]"));
                        betResult.setOdsForX(getIfPresent(driver, i, "//div[contains(@class, 'buttons-three-columns')]/button[2]"));
                        betResult.setOdsFor2(getIfPresent(driver, i, "//div[contains(@class, 'buttons-three-columns')]/button[3]"));
    ...
    private static List<WebElement> getContainers(WebDriver driver) {
        List<WebElement> containers = driver.findElements(By.xpath(CONTAINER_EL));
        if (containers.size() == 0) {
            sleep(10); //loads slowly sometimes
            containers = driver.findElements(By.xpath(CONTAINER_EL));
        }
        if (containers.size() == 0) {
    ...
    private static List<WebElement> getDivisions(WebDriver driver, int l) {
        ...
        do {
           attempt++;
           sleep(5);
           divisions = driver.findElements(By.xpath(xpath));
    ...
    private static String getIfPresentNotRoot(WebDriver driver, String xpath) {
        List<WebElement> res = driver.findElements(By.xpath(xpath));
        if (res.size() == 0) {
            return "";
        }
        if (res.size() > 1) {
        ...
        return res.get(0).getText();
    }

    private static String getIfPresent(WebDriver driver, int i, String xpath) {
        return getIfPresentNotRoot(driver, String.format("(//div[@class='subpage-game-row-mobile'])[%d]", i) + xpath);
    }
