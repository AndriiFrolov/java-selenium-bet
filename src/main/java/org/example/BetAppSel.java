...
public class BetAppSel {
    ...
    public static final String PATH_CSV = "/Users/Andrii_Frolov/Root/Projects/Sandbox/selenium-bet/selenium-bet/result.csv";
    public static final String PATH_EXCEL = "/Users/Andrii_Frolov/Root/Projects/Sandbox/selenium-bet/selenium-bet/result.xlsx";
    public static final String GAME_EL = "//div[@class='subpage-game-row-mobile']";
    public static final String LEAGUE_EL = "//div[contains(@class,'bet-card pitkaveto-container')]";
 ...
 //           "https://www.veikkaus.fi/fi/vedonlyonti?t=1-1-19_Kakkonen%20Lohko%20A",
  ...
  //          "https://www.veikkaus.fi/fi/vedonlyonti?t=1-1-8_Kansallinen%20Liiga"
    ...
    public static void main(String[] args) throws Exception {
        ...
        Configuration.headless = false; //Set to true if you don't want to see browser open
        ...
        for (int j = 0; j < urls.size(); j++) {
            ...
            ElementsCollection divisions = getDivisions();
            for (int k = 1; k <= divisions.size(); k++) {
                String league = "(" + LEAGUE_EL + ")[" + k + "]";
                ElementsCollection expandButton = $$(By.xpath(String.format("(//div[@class='bet-card__footer']/button)[%d]", k)));
                if (expandButton.size() > 1) {
                    log("!! Found expand buttons for league - " + expandButton.size());
                }
                if (expandButton.size() == 1) {
    ...
    private static ElementsCollection getDivisions() {
        ElementsCollection divisions = $$(By.xpath(LEAGUE_EL));
        if (divisions.size() == 0) {
            sleep(10); //loads slowly sometimes
            divisions = $$(By.xpath(LEAGUE_EL));
        }
        if (divisions.size() == 0) {
